...
class ViewController: UIViewController {
    ...
    override func loadView() {
        ...
        var componenets = Calendar.current.dateComponents([.hour, .minute], from: Date())
        componenets.hour = 8
        componenets.minute = 0
        wakeUpTime.date = Calendar.current.date(from: componenets) ?? Date()
        
        let sleepTitle = UILabel()
        sleepTitle.font = UIFont.preferredFont(forTextStyle: .headline)
        sleepTitle.numberOfLines = 0
        sleepTitle.text = "What is the mimimum amount of sleep you need?"
